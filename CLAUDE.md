...
- `com.sdg.inbox-auto` (`~/Library/LaunchAgents/com.sdg.inbox-auto.plist`) — **superseded, but still loaded & unpaused.** Runs [scripts/process-inbox-auto.sh](scripts/process-inbox-auto.sh): would auto-archive self-student happy-path submissions via a paranoid-restricted headless `claude -p` (`Read,Edit,Write` only, no MCP/Bash, `--add-dir` vault only, neutral cwd). ⚠️ The 2026-04-04 OAuth ban that would have 403'd its `claude -p` was **lifted ~2026-06** (memory `headless_claude_oauth_broken.md`), so it *might* run again — but headless `claude -p` from launchd is **untested on this Mac** and the live path is the sentinel + inline `/process-inbox` regardless. Recommend pausing (`touch ~/Code/sdg-html/.inbox-auto-paused`) so it stops double-polling the sentinel. Status: `bash scripts/inbox-auto-status.sh`. Log: `~/Library/Logs/sdg-inbox-auto.log`. Full notes: memory `inbox_auto_operations.md`.

**n8n workflow「自动录音转文字」** (id `ESa6oaWaxz9N7Oda-4zcY`, Schedule Trigger hourly) — Voice Memos → 阿里云 OSS (`n8n-recording` bucket, Shanghai) → Dashscope `fun-asr` ASR (switched from `paraformer-v2` 2026-06-18 after a 3-way bench — far better Chinese↔English code-switching, IC/UCL/LSE went from 0 hits to dozens; keeps speaker diarization) with speaker diarization + a 定制热词 vocabulary (`vocabulary_id` = `vocab-sdgf-…`, 500 校名/考试/顾问名 terms; see memory `stt_hotword_vocab_wired.md`) → POST raw transcript directly to Supabase `submissions` with `type='录音'`, `submitted_by='录音自动'`, `student_id=null`, `student_name_raw='【待确定，请和主管确认】'`. **No LLM in the n8n leg** — all intelligence (STT 纠错 / 角色判断 / 议题重组 / 学生归属) is deferred to manual `/process-inbox`, which must run vault `meeting-minutes` on the content. Replaces the old OneDrive `.md` write path (broken since 2026-05 vault move). Inspect via `curl -H "X-N8N-API-KEY:..." http://localhost:5678/api/v1/workflows/ESa6oaWaxz9N7Oda-4zcY` (API key in `~/.n8n/database.sqlite` `user_api_keys.label='ClaudeCode'`).
...
- [src/lib/auth.ts](src/lib/auth.ts) **verifies the Cloudflare Access JWT** (`Cf-Access-Jwt-Assertion`, via `jose` against the `sw985` team JWKS) before trusting the email header — the raw Vercel origin (`*.vercel.app`) is NOT behind CF Access, so without this a forged `Cf-Access-Authenticated-User-Email` header impersonates anyone incl. admin (was **live-exploitable**, fixed 2026-06-21). Then resolves the email to `Astro.locals.viewer` with a **60s** in-memory cache (per Vercel instance — admin demotions propagate within ~60s × N warm instances).
...
- **VIP contracts (旧称「私单」)** filtered unless `viewer.isAdmin || viewer.name === '王世杰'`. **Centralized 2026-06-21 into [src/utils/vip.ts](src/utils/vip.ts)** (`canSeeVIP` / `isVIP` / `filterVIP` / `VIP_CONTRACTS`) — every internal page imports it; the old 8-file inline duplication is gone. To change who sees VIP, or what marks a student VIP, edit ONLY that file (`viewer.name === '王世杰'` lives there now). `VIP_CONTRACTS` matches both `VIP` and legacy `私单`; any page fetching students must pass rows through `filterVIP(rows, viewer)` with `contracts` selected.
...
- **process-inbox** — archive `/internal/submissions` queue into the Obsidian vault, run **inline** in the foreground session. `SKILL.md` rewritten 2026-06 to **inline-first by design** (simpler + fully visible); the old headless `claude -p` subagent orchestration is retired (the 2026-04 OAuth ban that broke it was lifted ~2026-06, but inline stays the chosen path). Per submission: write a §3.1-scrubbed 沟通记录 (vault `meeting-minutes` treatment) + the 4 vault files (沟通记录 / 档案双链+最后沟通时间 / 日报-{中期顾问} / 待办看板) + run the meeting-minutes lint (`0 ERROR`), then mark Supabase `processed=true` and commit (explicit pathspecs, never `-A`). `type='录音'` (`student_name_raw='【待确定…】'`) → resolve the student via **TickTick calendar** (録音 filename timestamp ↔ that day's appointment; memory `recording_attribution_via_ticktick.md`) before archiving. Dedup/empty submissions → mark processed, no file. Triggered by the `com.sdg.inbox-sentinel` Bark push (see Cron above). Use for: cross-advisor rows, new clients, 録音, oversized content, anything wanting vault-skill smart treatment.
...
- **productivity-coach** — 王世杰 的私人效率教练（工具 = TickTick MCP）。触发：「规划今天/这周」「看看我手头/我满不满/还有多少空」「这事该我做吗/要不要外包/谁能接」「收工/下班」「我做完 X 了」。拉 TickTick 负载（用 `dida365` 原生 MCP——connector 已接、每会话 47 工具直接用；默认项目 中期带案25fall）→ 对照 vault `02_Project Manager/效率模型.md` 的累积工时算真实预算 + 标超载 → 对每件事 triage（🙋自己/👥交下属/💰外包/🙅拒绝-推迟）。**做完一件有代表性的活 → 顺口问世杰耗时、记进效率模型**（攒「一件事要多久」）。动 TickTick / 发消息前先打招呼，不自动发学生家长。**随手教练(ambient)**：在【接新事 / 做决定 / 收尾复盘 / 显露模式(反复亲做可外包事·镀金·沉没成本)】拐点、正常事做完后**可附一条 `〔教练〕` 一行探针**（≤2 句、可忽略）；他接→延伸 L1/L2、他挥手→立刻撤并记校准；**每话题至多一条、别打断执行流、读 room**。框架库 + 三级渐进见 skill `references/coaching-frameworks.md`。
...
### Pattern B — vault skill invocation (run INLINE by choice)

The old mechanism — spawning a headless `claude -p` into the vault — was killed by the 2026-04-04 OAuth ban, but that ban was **lifted ~2026-06** (memory `headless_claude_oauth_broken.md`): `claude -p` / Agent SDK draw from the normal subscription pool again. ⚠️ Still **volatile** (the policy flipped 5+ times in 2026) and **untested from launchd/cron on this Mac** — empirically verify before relying on it. Either way, run vault skills **inline** by choice (simpler, fully visible, no env-inheritance surprises).
...
- **TickTick** (`dida365`, `https://mcp.dida365.com`, HTTP + Bearer) — 任务/项目/习惯/专注/纪念日全套 47 工具；学生约谈在 `中期带案25fall`(id `65f7a21df346910636fcf4c1`)、时区 Asia/Shanghai。⚠️ **不在 `.mcp.json`，在 local config**（`~/.claude.json`，`claude mcp add` 加的，`dp_` token 私有、不入库）。用于 process-inbox 录音归属 + 纪要 Action Items 建任务 + productivity-coach 拉负载/排期/闭环。掉了就 `claude mcp add --transport http dida365 https://mcp.dida365.com --header "Authorization: Bearer <dp_token>"` 重接。

## Gotchas
- **⚠️ Vercel serverless runs Node 22.x (below 22.12), and Node 24 is NOT supported.** On that runtime `require(esm)` is OFF, so any dependency that is an ESM-only package `require()`'d from CJS on the server render path throws `ERR_REQUIRE_ESM` **on prod only** — `npm run dev` (Node 24, unbundled) renders it fine. This took down every markdown page (`students/[id]`, `daily-reports`, `advisors/[name]/weekly`, public `tools/personality/[id]`) on 2026-05-21 via `isomorphic-dompurify`→`jsdom@27`→ESM-only `parse5`. Server-side HTML sanitize is now `dompurify` + **`happy-dom`** ([src/lib/sanitize.ts](src/lib/sanitize.ts)) — do NOT reintroduce jsdom / isomorphic-dompurify, and do NOT use `linkedom` (it makes `DOMPurify.isSupported` undefined → `sanitize()` silently returns input **unsanitized**). To reproduce a Vercel-runtime bug locally, run the built `.vercel/output/.../entry.mjs` under Node 22.11. See memory `vercel_serverless_node22_and_bloat`.
- **⚠️ Static content routes MUST be prerendered** (`export const prerender = true` + `getStaticPaths` for dynamic routes). The Vercel adapter bundles **all SSR routes into ONE serverless function**, so an eager `import.meta.glob(..., { eager: true })` of bulk content (english articles/books) imported by any SSR route inflates the whole function → slower cold starts **site-wide** + a hard **250MB** function limit. English readers/books/both print routes + the static public/tool pages + `tools/guides/[slug]` are prerendered (function 40MB→27MB); Sanity/Supabase/auth pages (homepage, research/dialogues, all `/internal/*`) stay SSR intentionally. New static content → prerender it; keep `du -sh .vercel/output/functions/_render.func` small.
...
- **VIP (旧「私单」) visibility lives in [src/utils/vip.ts](src/utils/vip.ts)** (`VIP_CONTRACTS` / `canSeeVIP` / `isVIP` / `filterVIP`) — centralized 2026-06-21, the old inline-in-8-files `PRIVATE_CONTRACTS` duplication is gone. `VIP_CONTRACTS` matches both `VIP` and legacy `私单` (data migrated; 8 students). Adding a new VIP label or changing who sees VIP = edit that one file.
