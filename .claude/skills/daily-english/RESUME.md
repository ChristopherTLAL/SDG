...
```bash
# Regenerate the slim article-meta index. The internal student page's
# "recommend an article" widget reads src/data/english/_meta.generated.ts (NOT the
# live glob), so it stays stale until this runs. It writes under src/data/english/,
# so the `git add` below stages it automatically. (The public /tools/english pages
# auto-discover new articles via index.ts and need no regen.)
npm run gen-english-meta
...
- `src/data/english/_meta.generated.ts` — slim meta-only index (no article bodies), generated by `npm run gen-english-meta`; consumed by the internal student-detail "recommend article" widget so it doesn't bundle the full ~11MB article set. Regenerate after each batch (see commit step above)
